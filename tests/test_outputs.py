...
class TestPublicFixture:
    ...
    def test_rejected_rows_match_exactly(self, public_fixture):
        got = _load_text_lines(OUT_BAD)
        expected_path = os.path.join(HERE, "expected", "rejected_rows.log")
        expected = _load_text_lines(expected_path)
        assert got == expected, (
            f"rejected_rows.log mismatch: missing={sorted(expected - got)!r}, "
...
class TestGeneralization:
    ...
    def test_solution_reproduces_correctly_on_unseen_data(self, hidden_fixture):
        ...
        try:
            ...
            got_rejected = _load_text_lines(OUT_BAD)
            expected_rejected = _load_text_lines(
                os.path.join(HERE, "hidden", "rejected_rows.log")
            )
            assert got_rejected == expected_rejected, (
                f"hidden rejected_rows.log mismatch: missing={sorted(expected_rejected - got_rejected)!r}, "
                f"extra={sorted(got_rejected - expected_rejected)!r}"
            )
